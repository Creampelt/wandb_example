...
class WandbSummaryWriter(SummaryWriter):
    ...
    def __init__(self, log_dir: str, flush_secs: int, cfg) -> None:
    ...
    def store_config(self, env_cfg: dict, runner_cfg: dict, alg_cfg: dict, policy_cfg: dict) -> None:
    ...
    def add_scalar(
        ...
        self.run.log({self._map_path(tag): scalar_value, "local_step": global_step}, step=global_step)
    ...
    def save_model(self, model_path, iter) -> None:
        self.run.save(model_path, base_path=os.path.dirname(model_path))

    def save_file(self, path: str, iter: int | None = None) -> None:
        self.run.save(path, base_path=os.path.dirname(path))

    def log_video_files(self, log_name: str = "Video", video_subdir: str | None = "videos") -> None:
    ...
    def add_video(self, video_path: str, log_name: str = "Video") -> None:
        self.run.log({log_name: wandb.Video(video_path, format="mp4")})

    def callback(self, step: int) -> None:
